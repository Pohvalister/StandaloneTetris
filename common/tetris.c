...
void game_over() {
    ...
    char buffer3[10] = {'R', 'E', 'S', 'T', 'A', 'R', 'T', ':', ' ', 'r'};
    for (int j = 1; j < 11; ++j) {
...
void tetris_on_key_pressed(unsigned char key_code) {
    if (key_code != CODE_W && key_code != CODE_A && key_code != CODE_S && key_code != CODE_D) {
        new_operation = 1;
    } else {
        if (new_operation) {
            if (key_code == CODE_W) { //вверх
                change_version();
            } else if (key_code == CODE_S) { //вниз
                while (next_state()) {};
            } else if (key_code == CODE_D) { //вправо
                try_right();
            } else if (key_code == CODE_A){ //влево
                try_left();
            }
            new_operation = 0;
            full_print(0);
